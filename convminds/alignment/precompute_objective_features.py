...
import numpy as np
import logging
from convminds.benchmarks import get_benchmark_by_id
...
def main():
    ...
    parser.add_argument("--benchmark", type=str, required=True, help="Benchmark identifier (e.g. Pereira2018.243sentences)")
    ...
    benchmark = get_benchmark_by_id(args.benchmark)
    ...
    if hasattr(benchmark, "experiment"):
        experiment_name = benchmark.experiment.replace("sentences", "")
        output_filename = f"pereira2018_{experiment_name}_obj.npz"
    else:
        benchmark_slug = args.benchmark.lower().replace(".", "_").replace("-", "_")
        output_filename = f"{benchmark_slug}_obj.npz"
