...
from convminds.brainscore.model_helpers.preprocessing import prepare_context
from convminds.brainscore.utils import fullname
from convminds.brainscore.model_helpers.localize import localize_fed10


def get_layer_names(model_id: str, model_type: str = None) -> List[str]:
    """
    ...
    # Common layer name patterns for various architectures
    layer_patterns = {
        # GPT-like models
        "gpt2": "transformer.h.{}",
    ...
    if model_type not in layer_patterns:
        raise NotImplementedError(
            f"Unsupported model type: '{model_type}'. "
            f"Please add the layer naming pattern for this model type to 'get_layer_names'. "
            f"Supported types: {list(layer_patterns.keys())}"
        )

    layer_pattern = layer_patterns[model_type]

    # Determine the number of layers
    num_layers_attr_candidates = [
        "num_hidden_layers",  # Standard name
        "n_layer",            # Common in older GPT-like models
        "num_layers",         # Common in T5 and others
        "n_layers",           # Sometimes used
    ]
    
    num_layers = None
    for attr in num_layers_attr_candidates:
        if hasattr(config, attr):
            num_layers = getattr(config, attr)
            break
    
    if num_layers is None:
        raise ValueError(f"Could not determine the number of layers for model type '{model_type}'. "
                         f"Checked attributes: {num_layers_attr_candidates}")

    return [layer_pattern.format(i) for i in range(num_layers)]
